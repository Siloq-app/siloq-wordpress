...
class Siloq_Schema_Intelligence {
    ...
    public static function ajax_generate_schema() {
        ...
        // Write schema to post_meta (persists schema data for display + fallback)
        ...
        // Write to the Siloq Schema Architect DB table so inject_schema() outputs it in wp_head.
        // Without this step, schema is stored in meta but never output on the live page.
        if ( class_exists( 'Siloq_Schema_Architect' ) ) {
            foreach ( $schemas as $schema ) {
                $type = $schema['@type'] ?? 'Unknown';
                Siloq_Schema_Architect::save_schema( $post_id, $type, $schema, [
                    'confidence'       => 90,
                    'reason'           => 'Generated by Siloq Schema Intelligence',
                    'source'           => 'siloq_generate',
                    'validation_status'=> 'valid',
                ] );
            }
        }

        // Set _siloq_applied_types so the Schema tab shows "Applied ✓" immediately after generate.
        // G4 guardrail: auto-apply is safe here because schema goes to wp_head only —
        // it never modifies post_content or SEO plugin fields without user action.
        update_post_meta( $post_id, '_siloq_applied_types', wp_json_encode( $schema_types ) );
