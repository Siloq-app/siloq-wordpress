...
class Siloq_Connector {
    ...
    private function load_dependencies() {
        ...
        require_once SILOQ_PLUGIN_DIR . 'includes/class-siloq-case-study.php';
    ...
    private function init_components() {
        ...
        add_action( 'wp_ajax_siloq_send_scan_report',      array( 'Siloq_Scanner_Shortcode', 'ajax_send_scan_report' ) );
        add_action( 'wp_ajax_nopriv_siloq_send_scan_report', array( 'Siloq_Scanner_Shortcode', 'ajax_send_scan_report' ) );
    ...
    private function setup_hooks() {
        ...
        // Reposition + Rename intelligence (v1.5.193)
        add_action('wp_ajax_siloq_reposition_page',             array('Siloq_Admin', 'ajax_reposition_page'));
        add_action('wp_ajax_siloq_approve_rename',              array('Siloq_Admin', 'ajax_approve_rename'));
        add_action('wp_ajax_siloq_dismiss_rename',              array('Siloq_Admin', 'ajax_dismiss_rename'));

        // Page role override
        add_action('wp_ajax_siloq_set_page_role', array($this, 'ajax_set_page_role'));
        // Analyze single page via API
        add_action('wp_ajax_siloq_analyze_single_page', array('Siloq_Admin', 'ajax_analyze_single_page'));
        ...
        // Orphan internal link fix (intelligent)
        add_action('wp_ajax_siloq_get_orphan_suggestion',  array('Siloq_Admin', 'ajax_get_orphan_suggestion'));
        add_action('wp_ajax_siloq_auto_add_internal_link', array('Siloq_Admin', 'ajax_auto_add_internal_link'));
        ...
        add_action('wp_ajax_siloq_get_local_silos', array('Siloq_Admin', 'ajax_get_local_silos'));
        ...
        add_action('wp_ajax_siloq_publish_draft', array('Siloq_Admin', 'ajax_publish_draft'));
    ...
    public function enqueue_admin_assets() {
        ...
        if ($screen && ($screen->id === 'siloq-settings_page_siloq-dashboard' || (isset($_GET['page']) && $_GET['page'] === 'siloq-dashboard'))) {
            ...
            wp_localize_script('siloq-admin', 'siloqAdminData', array(
                ...
                'ajax_nonce' => wp_create_nonce('siloq_ajax_nonce'),
            ));
        }
    }
            ...
            $resp = wp_remote_request(
                ...
                array(
                    ...
                    'body'    => wp_json_encode(array(
                        ...
                        'page_type_classification' => in_array($role, array('hub', 'apex_hub'), true) ? 'money' : ($role ?: 'supporting'),
                    )),
                    'timeout' => 10,
                )
            );
